...
main (int argc, char** argv)
{
  ...
  viewer.addText ("Robot", 10, 10);

  //viewer1.setBackgroundColor (0, 0, 0);
  //viewer1.addCoordinateSystem (0.1);
  //viewer1.addText ("Cloud with ground", 10, 10);

  for (unsigned int j = 1; j <= 6; j++) //Esegui per tutte le immagini
  {
    ...
    {
        if(cloud_filtered->points[k].z < 2) //filtraggio z maggiori di 2m
        {
          //Assegnazione coordinate e colore ai punti
          PointT p;
    ...
    cloud_name<<"../data/es5/robot/"<<j<<".pcd";
    ...
    pcl::visualization::PointCloudColorHandlerRGBField<pcl::PointXYZRGB> rgb(robot);
    viewer.addPointCloud<PointT> (robot, rgb, "robot");

    //pcl::visualization::PointCloudColorHandlerCustom<PointT> red (plane_cloud, 255, 0, 0);
    //pcl::visualization::PointCloudColorHandlerRGBField<pcl::PointXYZRGB> rgb_rc(cloud_filtered);
    //viewer1.addPointCloud<PointT> (plane_cloud, red, "plane");
    //viewer1.addPointCloud<PointT> (cloud_filtered, rgb_rc, "no_plane");
