...
int main (int argc, char** argv)
{
	...
	//Detect the keypoints whith SHIFT operator
	...
	ne.setNumberOfThreads(2); 	// set number of threads when using OpenMP 
	ne.compute (*normals);
	std::cout << "done." << std::endl;
	...
	fpfh.setNumberOfThreads(2); 	// set number of threads when using OpenMP
	fpfh.compute (*fpfhs);
	std::cout << "done." << std::endl;
	...
	viewer.addPointCloud<pcl::PointXYZRGB> (KeyPoints, rgb, "input_cloud");
	//viewer.addPointCloudNormals<pcl::PointXYZRGB, pcl::Normal>(cloud, normals, normalsVisualizationStep, normalsScale, "normals");
